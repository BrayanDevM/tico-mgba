...
void TicoOverlay::RenderGame(ImDrawList *dl, ImVec2 displaySize, unsigned int texture,
    ...
    if (m_displayMode == MgbaDisplayMode::Integer) {
        ...
        case MgbaDisplaySize::Stretch: dstWidth = displaySize.x; dstHeight = displaySize.y; break;
        case MgbaDisplaySize::_4_3: {
...
void TicoOverlay::RenderSettingsMenu(ImDrawList *dl, ImVec2 displaySize) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (i == 0) { label = tr("emulator_display_mode"); value = (m_displayMode == MgbaDisplayMode::Integer) ? tr("emulator_integer") : tr("emulator_display"); }
        else if (i == 1) {
            label = tr("emulator_size");
            if (m_displayMode == MgbaDisplayMode::Integer) {
                switch (m_displaySize) { case MgbaDisplaySize::_1x: value="1x"; break; case MgbaDisplaySize::_2x: value="2x"; break; default: value=tr("emulator_auto"); break; }
            } else {
                switch (m_displaySize) { case MgbaDisplaySize::Stretch: value=tr("emulator_stretch"); break; case MgbaDisplaySize::_4_3: value="4:3"; break; case MgbaDisplaySize::_16_9: value="16:9"; break; default: value=tr("emulator_original"); break; }
            }
        } else if (i == 2) {
            label = tr("emulator_shader");
            const char *shaderNames[] = {"None", "LCD", "xBRZ", "Eagle", "Dot", "LCD Grid V2"};
            value = shaderNames[m_shaderSelection % 6];
        }
        ImU32 textColor;
...
bool TicoOverlay::HandleInput(SDL_GameController *controller) {
    ...
    if (togglePressed && !m_toggleHeld && debounced) {
        ...
        else if (m_currentMenu == OverlayMenu::SaveStates || m_currentMenu == OverlayMenu::Settings || m_currentMenu == OverlayMenu::Tools || m_currentMenu == OverlayMenu::Cheats) { m_currentMenu = OverlayMenu::QuickMenu; m_animTimer = 0.4f; }
        else Hide();
        return true;
    }
    ...
    if (dirChanged && m_currentMenu == OverlayMenu::Settings) {
        if (m_settingsSelection == 0) {
            m_displayMode = (m_displayMode == MgbaDisplayMode::Display) ? MgbaDisplayMode::Integer : MgbaDisplayMode::Display;
            m_displaySize = (m_displayMode == MgbaDisplayMode::Integer) ? MgbaDisplaySize::Auto : MgbaDisplaySize::_4_3;
            ApplyScalingSettings(true);
        } else if (m_settingsSelection == 1) {
            if (m_displayMode == MgbaDisplayMode::Integer) {
                int s = (int)m_displaySize + dir; if (s < 4) s = 6; if (s > 6) s = 4;
                m_displaySize = (MgbaDisplaySize)s;
            } else {
    ...
    if (confirm && !m_confirmHeld && debounced) {
        ...
        if (m_currentMenu == OverlayMenu::QuickMenu) {
            ...
            if (m_settingsSelection == 0) {
                ...
                if (m_displayMode == MgbaDisplayMode::Integer) { int s = (int)m_displaySize; s = (s >= 6) ? 4 : s+1; m_displaySize = (MgbaDisplaySize)s; }
                else { int s = (int)m_displaySize; s = (s >= 3) ? 0 : s+1; m_displaySize = (MgbaDisplaySize)s; }
                ApplyScalingSettings(true);
            } else if (m_settingsSelection == 2) {
...
#ifdef __SWITCH__
    std::string configPath = "sdmc:/tico/config/cores/mgba.jsonc";
#else
    std::string configPath = "tico/config/cores/mgba.jsonc";
#endif
    ...
    if (file.is_open()) {
        ...
        if (!j.is_discarded()) {
            ...
            if (j.contains("display_size") && j["display_size"].is_string()) {
                ...
                else if (v=="16:9") m_displaySize = MgbaDisplaySize::_16_9;
                else if (v=="1x") m_displaySize = MgbaDisplaySize::_1x;
...
void TicoOverlay::SaveCoreSettings() {
    ...
    j["display_mode"] = (m_displayMode == MgbaDisplayMode::Integer) ? "Integer" : "Display";
    const char *sizeStr = "4:3";
    switch (m_displaySize) {
    case MgbaDisplaySize::Stretch: sizeStr="Stretch"; break; case MgbaDisplaySize::_4_3: sizeStr="4:3"; break;
    case MgbaDisplaySize::_16_9: sizeStr="16:9"; break;
    case MgbaDisplaySize::_1x: sizeStr="1x"; break; case MgbaDisplaySize::_2x: sizeStr="2x"; break;
    case MgbaDisplaySize::Auto: sizeStr="Auto"; break; default: break;
    }
